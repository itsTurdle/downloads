...
def main():
    ...
    ok, fail = [], []
    for y in years:
        url = f"{BASE}/download-free-forex-data/?/ascii/1-minute-bar-quotes/eurusd/{y}"
        try:
        ...
        time.sleep(0.8)
    for m in range(1, 13):
        url = f"{BASE}/download-free-forex-data/?/ascii/1-minute-bar-quotes/eurusd/2026/{m}"
        try:
            blob = download_zip(url)
            if blob:
                n = ingest(blob)
    ...
    for k in sorted(hourly):
        st = time.strptime(k, "%Y%m%d %H")
        epoch = calendar.timegm(st) + 5 * 3600
    ...
    t0 = rows[0][0]
    pack["hr"] = dict(t0=t0, dt=[(t - t0) // 3600 for t, _ in rows],
                      e=[round(c, 5) for _, c in rows])
    js = json.dumps(pack, separators=(",", ":"))
    open(f"{SP}/fx/pack.json", "w").write(js)
